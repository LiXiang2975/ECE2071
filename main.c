...
int main(void)
{
  ...
  HAL_ADC_Start_IT(&hadc1); //start ADC in interrupt mode here
  /* USER CODE END 2 */
...
//		//Print raw_ADC value into string_1
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  {
//	  HAL_GPIO_TogglePin(GPIOB, LD3_Pin); // Toggle LED
      raw_ADC_value = HAL_ADC_GetValue(&hadc1);
    // Process or store adc_value as needed
    ...
    //Extract least significant 8 bits of ADC data
    		...
    		//Transmit 16-bit ADC data through UART
    //		HAL_UART_Transmit(&huart2, (uint8_t*)raw_ADC_value_1,2,10);
    		HAL_SPI_Transmit_IT(&hspi1, raw_ADC_value_1, 2);


    		HAL_ADC_Stop_IT(&hadc1); //Stop ADC
//    		HAL_GPIO_TogglePin(GPIOB, LD3_Pin); // Toggle LED
